...
public class Main5 {
    public static void main(String[] args) {
        ...
        list.stream().filter(person -> person.getAge()==20).map(person -> person.getName() + " 입니다.").forEach(System.out::println);
        ...
        int sum = list.stream().mapToInt(person -> person.getAge()).filter(integer -> integer>20).reduce(0,((integer, integer2) -> integer += integer2));
